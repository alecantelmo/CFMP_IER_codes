...
%    approximated by simulating the model for 10000 periods with the 3rd order TP
%    solution.
...
% models=models(end-1:end); %take a specific model

%%% all solutions
Orders_for_Perturbation=[1,2,3,4,5];
Orders_for_TaylorProjection=[1,2,3];
Orders_for_Smolyak=[1,2,3];
...
% all_versions={'ver0','ver1','ver2','ver3','ver4'}; % do both and robustness tests
...
for veri=1:length(all_versions);
    ...
    for modeli=1:length(models)
        ...
        load([main_folder '\files_for_TaylorProjection\' models{modeli} '\order3\model'],'model');
...
%             save([main_folder '\performance\' version '\' models{modeli} '\simulation'],'results_*','x_tp*','x_pert*','x_smol*','nu_*')
